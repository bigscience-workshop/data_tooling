...
def write(result: Union[str, Iterable[Dict]], mode: str,
          index: Optional[Dict], out: TextIO):
    '''
...
def process_file(infile: str, outfile: str, lang: str,
                 country: List[str] = None,
                 tasks: List[str] = None, all_tasks: bool = False,
                 split: str = 'line', mode: str = 'replace',
                 template: str = None, debug: bool = False,
                 show_tasks: bool = False, show_stats: bool = False) -> Dict:
    '''
    Process PII tasks on a file
    '''
    # Create the object
    proc = PiiManager(lang, country, tasks, all_tasks=all_tasks,
                      mode=mode, template=template, debug=debug)
    if show_tasks:
