...
@pytest.mark.parametrize("mode", ["replace", "tag", "extract"])
def test10_line(mode):
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        stats = process_file(datafile('orig.txt'), f.name, 'en', tasks=tasks,
                             mode=mode)
        exp = {'calls': 3, 'CREDIT_CARD': 1, 'BITCOIN_ADDRESS': 1}
        assert stats == exp

        name = mode + ('.txt' if mode != 'extract' else '-line.ndjson')
...
def test20_block(mode):
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        name = mode + ('.txt' if mode != 'extract' else '-block.ndjson')
        exp = readfile(datafile(name))
        got = readfile(f.name)
        assert got == exp
...
def test30_sentence(mode):
    ...
    with tempfile.NamedTemporaryFile() as f:
        tasks = [PiiEnum.CREDIT_CARD, PiiEnum.BITCOIN_ADDRESS]
        stats = process_file(datafile('orig.txt'), f.name, 'en', tasks=tasks,
                             mode=mode, split="sentence")
        exp = {'calls': 2, 'CREDIT_CARD': 1, 'BITCOIN_ADDRESS': 1}
        assert stats == exp

        name = mode + ('.txt' if mode != 'extract' else '-sentence.ndjson')
        exp = readfile(datafile(name))
        got = readfile(f.name)
        assert got == exp
