...
def visualization(path_data, lang, num_docs, num_docs_for_words):
    ...
    # OPENING DATA AND DEFINITION OF DATAFRAMES

    with open(path_data) as json_file:
        data = json.load(json_file)

    num_docs = min(num_docs, len(data))
    num_docs_for_words = min(num_docs_for_words, len(data))
    ...
    st.subheader(f"Discarded documents: {len(data_not_keep)} docs ({len(data_not_keep) / num_docs * 100:.2f}%)")
    ...
    st.subheader(f"Retained documents: {len(data_keep)} docs ({len(data_keep) / num_docs * 100:.2f}%)")
    st.markdown("Click on a column to sort by it, place the cursor on the text to display it.")
    ...
    st.subheader(f"Discarded words: {len(words_not_keep)} words ({len(words_not_keep) / len(words_data) * 100:.2f}%)")
    st.markdown("Click on a column to sort by it, place the cursor on the text to display it.")
    st.dataframe(words_not_keep)

    words_keep = words_data.loc[cond_words]
    st.subheader(f"Retained words: {len(words_keep)} words ({len(words_keep) / len(words_data) * 100:.2f}%)")
    st.markdown("Click on a column to sort by it, place the cursor on the text to display it.")
    ...
    st.header("Distributions of the parameters")
