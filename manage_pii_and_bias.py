...
# from a webcrawled index of licensed dataset is exposed. 
  ...
  @staticmethod
  def annotate_PII_job(batch, target_lang, shared_dir):
    for line in batch:
      ...
      if target_lang != pii_manager.langid_ext(text_to_anonymize,):
      ...
      if target_lang !='en':
        ...
        pii_manager.text_to_anonymize_en = translate(text_to_anonymize, fr=target_lang, to='en')
        print(text_to_anonymize_en)
      analyzer_results = pii_manager.analyzer.analyze(text=text_to_anonymize_en, language='en')
      print (analyzer_results)
      label_to_mask = {}
      pii_test_set = pii_manager.template_lambda(analyzer_results, text_to_anonymize_en, target_lang=target_lang, already_replaced={}, label_to_mask=label_to_mask) 
      print (pii_test_set.text)
      anonymized_text0 =  pii_manager.back_trans(pii_test_set.text, fr='en', to=target_lang)
      (blocks2, score, score0) =  get_aligned_text(text_to_anonymize, anonymized_text0, target_lang)
      print (blocks2)
      #since we know what slots we are replacing, we can align with the original sentence in the original language, and then insert the fake value for the slots. 
      mask_to_pii_encrypt = {}
      ...
      anonymized_text1 = pii_manager.anonymize_with_faker(text_to_anonymize, label_to_val, target_lang) # maybe we run this through back_trans using some intermediate lang??
      print ('aonymized method 1', anonymized_text1)
      label_to_val = None # make sure we delete this dict as it has the actual PII
      anonymized_text2 = pii_manager.anonymize_with_faker(anonymized_text0, label_to_mask, target_lang) # maybe we run this through back_trans using some intermediate lang??
      print ('aonymized method 2', anonymized_text2)
      anonymized_results = pii_manager.anonymize_faker_lambda(analyzer_results, text_to_anonymize_en, target_lang=target_lang, already_replaced={})
      print ('aonymized method 3',  pii_manager.back_trans(anonymized_results.text, fr='en', to=target_lang))
      # method 2 appears to be better than 1, and 1 better than method 3  
      # TODO, do probing using Question Gen.

  @classmethod
  def annotate_PII(cls, target_lang, shared_dir):
    pii_manager = cls(target_lang)
    #load the oscar dataset
    oscar_gizp_file = target_lang # open using fsspec
    dataset = Datastore.add_igzip(oscar_gizp_file)
    dataset.map(annotate_PII_job, batched=True, batch_size=100000, fn_kwargs={'target_lang': target_lang, 'shared_dir':})
    
if __name__ == "__main__": 
  global dask_node_id, dask_client
  if launch_dask("/content/drive/Shareddrives/BigScience/dask_scheduler.txt") is None:
    raise RuntimeError(f"this must be run on the dask master node 0. this is node {dask_node_id}")
  bigscience_lang = ['sl', 'hr', 'mk', 'bg', 'pl', 'uk', 'sk', 'cs', 'be', 'ru', 'fi', 'eu', 'zh', 	'vi', 'fr', 'es', 'id', 'pt', 'ca', 'ar', 'ur', 'hi', 'bn', 'yo', 'sw', 'ng', 'rw', 'ig', 'ha', ] #en
  results_files = list(wait_until_files_loaded([s2.result() for s2 in  \
                                                dask_client.map(PII_NER_Manager.annotate_PII, [(target_lang, shared_dir) for target_lang, shared_dir in zip(bigscience_lang, ["/content/drive/Shareddrives/BigScience"]*len(bigscience_lang))])])
