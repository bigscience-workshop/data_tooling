...
class MinHashProcessor (Processor):
  ...
  @staticmethod
  def shingles(textArr, word_ngram=5):
      return set(textArr[head:head + word_ngram]
                for head in range(0, len(text) - word_ngram))
  ...
  @staticmethod
  def compute_fingerprint(line, key, hasher):
      try:
          myjson = json.loads(line)
          url = myjson[key]
          textArr = myjson['text'].split()
          fingerprint = hasher.fingerprint(textArr)
  ...
  @staticmethod
  def compute_jaccard(each_bin, hasher, num_bins, doc_row_ids, start_time_local):
      ...
      return remove_urls_list, deduped_local, counter_local, each_bin

  def find_pair_rows_parallel(self, args, doc_row_ids):
      start_time = time.time()
      deduped, counter = 0, 0

      # compute jaccards of buckets in bin in parallel (parallelism
      # limited to # of bins)
      num_bins = len(self.lshcache.bins)
      ...
      for similar_list, deduped_local, counter_local, bin in compute_jaccard_iter:
          ...
          add_to_similarity_list(similar_list, f_out)
