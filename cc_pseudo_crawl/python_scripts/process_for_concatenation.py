...
def get_args():
    ...
    parser.add_argument("--shard_max_size", type=int, default=1_000_000_000, help="max shard size, default 10GB")
    parser.add_argument(
        "--save-path",
        type=Path,
        required=True
...
def save_dataset(ds: Dataset, save_path: Path, shard_max_size: int):
    ...
    shards = [ds.shard(num_shards=num_shards, index=shard_id) for shard_id in range(num_shards)]
    ...
    logger.info(f"Now you need to manually update update states.json to concatenate all data files")
    ...
    with Pool(args.load_num_proc) as pool:
        datasets = pool.imap(
            ...
            [
                ...
                for (ds_name, ratio), child_seed in
                zip(datasets_with_ratios, seed.spawn(len(datasets_with_ratios)))
            ],
        )
        datasets = [
            ds
