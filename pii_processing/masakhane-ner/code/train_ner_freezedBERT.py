...
class Classifier(nn.Module):
    ...
    def __init__(self, num_classes, input_dim=768, hidden_size=64, pooler_dropout=0.3):
        ...
        )  # 2*hidden_size because bidirectional is True
    ...
    def forward(self, sent_rep):
        ...
        packed_input = pack_padded_sequence(
            sent_rep, input_lengths.cpu().numpy(), batch_first=True
        )
        ...
        final_feature_map = self.dropout(
            output
        )  # shape=(num_layers * num_directions, 64, hidden_size)
...
def train(
    args, train_dataset, bert_model, model, tokenizer, labels, pad_token_label_id
):
    """Train the model"""
...
def evaluate(
    args, bert_model, model, tokenizer, labels, pad_token_label_id, mode, prefix=""
):
...
def main():
    ...
    if (args.do_train or args.do_finetune) and (
        ...
        torch.save(args, os.path.join(args.output_dir, "training_args.bin"))
        torch.save(
            model_to_save.state_dict(), os.path.join(args.output_dir, "bert_lstm.model")
        )
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.load_state_dict(
                torch.load(os.path.join(args.output_dir, "bert_lstm.model"))
            )
            result, _ = evaluate(
                args,
                bert_model,
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        result, predictions = evaluate(
            args, bert_model, model, tokenizer, labels, pad_token_label_id, mode="test"
        )
        # Save results
