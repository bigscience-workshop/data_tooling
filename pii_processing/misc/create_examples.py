            ...
            for d in (dataset["train"],):
                for idx, data in enumerate(d):
                    ...
                    if (
                        random.choice([0, 1])
                        and not has_a_name
                        and "mr." not in l2_lower
                        and "ms." not in l2_lower
                        and "mrs." not in l2_lower
                        and "president" not in l2_lower
                        and "governor" not in l2_lower
                        and "mayor" not in l2_lower
                    ):
                        continue
                    if (
                        len(l2Arr) > 10
                        and len(l2Arr) < 50
                        and (score <= 0.5 or random.randint(0, 10) == 0)
                    ):  # having too much toxic content may skew the data
                        if (
                            has_a_name
                            or "mr." in l2_lower
                            or "ms." in l2_lower
                            or "mrs." in l2_lower
                            or "senator" in l2_lower
                            or "president" in l2_lower
                            or "governor" in l2_lower
                            or "mayor" in l2_lower
                        ):
    ...
    with open("english_cleaned.tsv", "w", encoding="utf8") as o:
        with open("english.tsv", "rb") as f:
            ...
            while True:
                ...
                if prev != "" and (
                    l2 == prev2
                    or (len(prev) > 10 and len(l) > 10 and prev2[:10] == l2[:10])
                ):
                    if len(l) > len(prev):
                        prev = l
                    continue
                ...
                if prev:
                    ...
                    while len(sent) > 3:
                        ...
                        if (
                            sent2
                            and sent2[0]
                            and sent2[0][0].lower() not in "abcdefghijklmnopqrstuvwxyz"
                        ):
                            prev = l
                            continue
                        ...
                        sent2 = ". ".join(sent2) + "."
                        sent2 = (
                            sent2.strip().replace("  ", " ").replace(" .com", ".com")
                        )
                        if len(sent2) < 20:
                            prev = l
                            continue
                    ...
                    if (
                        sent
                        and sent[0]
                        and sent[0][0].lower() not in "abcdefghijklmnopqrstuvwxyz"
                    ):
                        prev = l
                        continue
