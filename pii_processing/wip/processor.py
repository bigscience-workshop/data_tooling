...
class Processor(OntologyManager):
    ...
    def __init__(
        ...
        tmp_dir=None,
        max_word_len=4,
        compound_word_step=3,
        strip_chars=None,
        upper_ontology=None,
        x_lingual_lexicon_by_prefix_file="lexicon_by_prefix.json.gz",
        target_lang_config_file=None,
        x_lingual2ner_file=None,
        connector="_",
        en_spacy_models=[],
        label2label=None,
    ):
        super().__init__(
            ...
            compound_word_step=compound_word_step,
            strip_chars=strip_chars,
            upper_ontology=upper_ontology,
            x_lingual_lexicon_by_prefix_file=x_lingual_lexicon_by_prefix_file,
            target_lang_config_file=target_lang_config_file,
            x_lingual2ner_file=x_lingual2ner_file,
            connector=connector,
        ...
        if True:
            ...
            if target_lang in self.lang2ner_model:
                ...
                self.ner_model_pipeline = pipeline(
                    "ner",
                    model=model,
    ...
    def _spacy_ner_coref(
        self, text, nlp, chunk2ner, chunk2ref, ref2chunk, row_id=0, doc_id=0
    ):
        """
        ...
        for coref, spans in corefs:
            ...
            if new_spans:
                ...
                ref2chunk[new_coref[0].lower()] = list(
                    set(list(ref2chunk.get(new_coref[0].lower(), [])) + new_spans)
                )
        ...
        if True:
            for cl in list(doc._.coref_clusters):
                cluster_text_list = {
                    m.text.lower() for m in cl.mentions if m.text != "US"
                }
                found = False
        ...
        return self._cleanup_chunks(
            ...
            doc_id,
            doc,
            mention_access_fn=lambda a: a.text,
        )
    ...
    def _hf_ner(self, text, chunk2ner, chunk2ref, ref2chunk, row_id=0, doc_id=0):
        ...
        return self._cleanup_chunks(
            text, chunk2ner, chunk2ref, ref2chunk, row_id, doc_id
        )

    def _rule_based_ner_coref(
        self,
        text,
        chunks2,
        chunk2ner,
        ...
        if do_ontology:
            ...
            word2label = {
                a[0].replace(connector, " "): b for a, b in dat["chunk2ner"].items()
            }
            for x, label2 in word2label.items():
                ...
                if "<" in x and ">" in x:
                    x = " ".join(
                        [
                            a if a not in label2word else label2word[a]
        ...
        for rng in range(number_regex_cycle):
            ...
            for spanIdx, mention in enumerate(chunks2):
                ...
                wordsDict = {
                    s.strip(self.strip_chars): 1 for s in span_str.lower().split()
                }  # TODO, we can stem the surrounding word and wordsDict
                ner_regex_list1 = []
                ner_regex_list2 = []
                print(ner_regexes)
                ...
                for label2, regex0 in ner_regex_list1 + ner_regex_list2:
                    for x in regex0.findall(span_str):
                        ...
                        label2word["<" + label2.upper() + str(idx) + ">"] = (
                            x.strip(),
                            label2,
                        )
                        idx += 1
                ...
                for word in wordArr:
                    ...
                    if not coref and word in pronouns:
                        for idx in [i for i in self.coref_window if i < 0]:
                            if (
                                chunk2ner.get(chunks[idx])
                                and chunks[idx][0].lower() not in pronouns
                            ):
                                if (
                                    word in person_pronouns
                                    and "PERSON"
                                    not in self.ontology.get(
                                        chunk2ner.get(chunks[idx]), []
                                    )
                                ):
                                    continue
                    ...
                    if not coref and word in pronouns:
                        for idx in [spanIdx + i for i in self.coref_window]:
                            if (
                                idx >= 0
                                and idx < len_chunks2
                                and chunk2ner.get(chunks2[idx])
                                and chunks2[idx][0].lower() not in pronouns
                            ):
                                if (
                                    word in person_pronouns
                                    and "PERSON"
                                    not in self.ontology.get(
                                        chunk2ner.get(chunks2[idx]), []
                                    )
                                ):
                                    continue
                    ...
                    if word in label2word or word in word2label:
                        if prev_words:
                            ...
                            self.add_chunks_span(
                                chunks,
                                (
                                    new_word,
                                    0 if not chunks else chunks[-1][2] + 1,
                                    len_new_word
                                    if not chunks
                                    else chunks[-1][2] + 1 + len_new_word,
                                    row_id,
                                    doc_id,
                                ),
                                None,
                        ...
                        new_word = (
                            label2word[word][0]
                            if word in label2word
                            else word.replace(connector, " ")
                        )
                        len_new_word = len(new_word)
                        new_label = (
                            label2word[word][1]
                            if word in label2word
                            else (word2label[word] if word in word2label else label)
                        )
                        self.add_chunks_span(
                            chunks,
                            (
    ...
    def _cleanup_chunks(
        ...
        for coref, seq in ref2chunk.items():
            labels = [
                chunk2ner[mention]
                for mention in seq
                if mention in chunk2ner and chunk2ner[mention] != "NOUN"
            ]
            if labels:
                label = Counter(labels).most_common()[0][0]
                for mention in seq:
                    if mention in chunk2ner and label not in self.upper_ontology.get(
                        chunk2ner[mention], []
                    ):
        ...
        chunks = [
            a for a in chunk2ner.items() if a[0][-2] == row_id and a[0][-1] == doc_id
        ]
            ...
            if not chunks2 or (chunks2[-1][2] <= mention[1]):
                if not chunks2 or chunks2[-1][2] < mention[1]:
                    self.add_chunks_span(
                        ...
                        (
                            mention_access_fn(
                                doc[0 if not chunks2 else chunks2[-1][2] : mention[1]]
                            ),
                            0 if not chunks2 else chunks2[-1][2],
                            mention[1],
                            row_id,
                            doc_id,
                        ),
                ...
                if s0:
                    self.add_chunks_span(
                        chunks2,
                        (s0, old_mention[1], mention[1], row_id, doc_id),
                        None,
                        oldLabel
                        if s0 in pronouns or (len(s0) > 1 and s0 not in self.stopwords)
                        else None,
                        oldAnaphore
                        if s0 in pronouns or (len(s0) > 1 and s0 not in self.stopwords)
                        else None,
                ...
                self.add_chunks_span(
                    ...
                    label,
                    oldAnaphore
                    if not chunk2ref.get(mention)
                    else chunk2ref.get(mention),
                    chunk2ner,
                    chunk2ref,
                    ref2chunk,
                )
                if len(sArr) > 1:
                    ...
                    if s1:
                        self.add_chunks_span(
                            chunks2,
                            (s1, mention[2], old_mention[2], row_id, doc_id),
                            None,
                            oldLabel
                            if s1 in pronouns
                            or (len(s1) > 1 and s1 not in self.stopwords)
                            else None,
                            oldAnaphore
                            if s1 in pronouns
                            or (len(s1) > 1 and s1 not in self.stopwords)
        ...
        if chunks2[-1][2] < len_doc:
            self.add_chunks_span(
                chunks2,
                (
                    mention_access_fn(doc[chunks2[-1][2] :]),
                    chunks2[-1][2],
                    len_doc,
                    row_id,
                    doc_id,
        ...
        for spanIdx, mention in enumerate(chunks2):
            ...
            self.add_chunks_span(
                ...
                (
                    new_word,
                    0 if not chunks else chunks[-1][2] + 1,
                    len_new_word if not chunks else chunks[-1][2] + 1 + len_new_word,
                    row_id,
                    doc_id,
                ),
                None,
    ...
    def analyze_with_ner_coref(
        ...
        ref2chunk=None,
        chunk2ref=None,
        do_preprocess_basic=False,
        do_post_process_regex=False,
        do_spacy=True,
        ...
        assert (
            target_lang == self.target_lang or target_lang == "en"
        ), "Can only processes in English or the target_lang"
        ...
        if is_cjk and self.t5_tokenizer is not None and self.detect_cjk(text):
            text = (
                " ".join(self.mt5_tokenize(text))
                .replace(self.mt5_underscore + " ", self.mt5_underscore)
                .strip()
            )
        chunks = []
        chunks2 = []
        ...
        if do_preprocess_basic:
            ...
            text, chunks, chunk2ner, chunk2ref, ref2chunk = self._rule_based_ner_coref(
                text,
                [(text, 0, len(text), row_id, doc_id)],
                chunk2ner,
                chunk2ref,
                ref2chunk,
                row_id,
        ...
        if target_lang == "en" and do_spacy:
            for nlp in self.en_spacy_models:
                text, chunks2, chunk2ner, chunk2ref, ref2chunk = self._spacy_ner_coref(
                    text,
                    nlp,
                    chunk2ner,
                    chunk2ref,
        ...
        if do_transformer and self.ner_model_pipeline is not None:
            text, chunks3, chunk2ner, chunk2ref, ref2chunk = self._hf_ner(
        ...
        # do one final cleanup and resolve any conflicts
            ...
            text, chunks, chunk2ner, chunk2ref, ref2chunk = self._cleanup_chunks(
                text, chunk2ner, chunk2ref, ref2chunk, row_id, doc_id
            )
    ...
    def process(self, text="", batch=None, *args, **argv):
        ...
        if type(batch[0]) is str:
            if batch[0][0] == "{" and batch[0][-1] == "}":
                ...
                batch = [
                    {"id": _id + row_id, "doc_id": doc_id, "text": text}
                    for _id, text in enumerate(batch)
                ]
        if "do_round_trip_trans" in argv and target_lang != "en":
            ...
            batch_en = [
                self.analyze_with_ner_coref(text, target_lang="en") for text in batch_en
            ]
            batch_trans = [
                self.trans.translate(text, target_lang=target_lang) for text in batch_en
            ]
            batch = [
                self.analyze_with_ner_coref(
                    text,
                    target_lang=target_lang,
                    chunk2ner=chunk2ner,
                    ref2chunk=ref2chunk,
                    chunk2ref=chunk2ref,
                    do_preprocess_basic=argv.get("do_preprocess_basic", False),
                    do_post_process_regex=argv.get("do_post_process_regex", False),
                    do_spacy=argv.get("do_spacy", True),
                    do_transformer=argv.get("do_transformer", False),
                )
                for text in batch
            ]
