...
class OntologyManager:
    ...
    default_data_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), os.path.pardir, "lexicon")
    )
    ...
    def load_x_lingual_lexicon_from_prefix_file(
        self, x_lingual_lexicon_by_prefix_file="lexicon_by_prefix.json.gz"
    ):
        data_dir = self.data_dir
        tmp_dir = self.tmp_dir
        if x_lingual_lexicon_by_prefix_file is not None:
            if not os.path.exists(x_lingual_lexicon_by_prefix_file):
                x_lingual_lexicon_by_prefix_file = (
                    f"{data_dir}/{x_lingual_lexicon_by_prefix_file}"
                )
            if not os.path.exists(x_lingual_lexicon_by_prefix_file):
            ...
            if x_lingual_lexicon_by_prefix_file.endswith(".gz"):
                ...
                self.x_lingual_lexicon_by_prefix = json.load(
                    open(x_lingual_lexicon_by_prefix_file, "rb")
                )
    ...
    def save_x_lingual_lexicon_prefix_file(
        self, x_lingual_lexicon_by_prefix_file="lexicon_by_prefix.json.gz"
    ):
        ...
        x_lingual_lexicon_by_prefix_file = x_lingual_lexicon_by_prefix_file.replace(
            ".gz", ""
        )
        ...
        json.dump(
            self.x_lingual_lexicon_by_prefix,
            open(x_lingual_lexicon_by_prefix_file, "w", encoding="utf8"),
            indent=1,
        )
    ...
    def load_target_lang_config(self, target_lang_config_file=None, target_lang=None):
        ...
        if "ner_regexes" in self.target_lang_config:
            ...
            ner_regexes = [
                regex
                for regex in self.target_lang_config["ner_regexes"]
                if regex[0] != "PERSON" and regex[0] in self.upper_ontology
            ]
        ...
        self.gender2en_pronoun = dict(
            itertools.chain(
                *[[(b, a) for b in lst] for a, lst in self.en_pronoun2gender.items()]
            )
        )
        self.pronoun2en_pronoun = dict(
            itertools.chain(
                *[[(b, a) for b in lst] for a, lst in self.en_pronoun2pronoun.items()]
            )
        )
        self.title2en_pronoun = dict(
            itertools.chain(
                *[[(b, a) for b in lst] for a, lst in self.en_pronoun2title.items()]
            )
        )
        self.religion2person = dict(
            itertools.chain(
                *[[(b, a) for b in lst] for a, lst in self.person2religion.items()]
            )
        )
        self.coref_window = self.target_lang_config.get(
            "coref_window", [-1, -2, 1, 2]
        )  # maybe this should be a parameter and not in the ontology

        # now specialize the ontology for target_lang and have no limit on the size of the words
        target_lang_ontology = {}
        for label, words in (
            self.target_lang_config
            if type(self.target_lang_config) is list
            else self.target_lang_config.items()
        ):
            if label in self.upper_ontology:
                for word in words:
        ...
        self.add_to_ontology(
            target_lang_ontology,
            max_word_len=100000,
            onto_name=os.path.split(target_lang_config_file)[-1].split(".")[0],
        )
    ...
    def save_target_lang_config(self, target_lang_config_file):
        ...
        json.dump(
            self.target_lang_config,
            open(f"{data_dir}/{target_lang_config_file}", "w", encoding="utf8"),
            indent=1,
        )
    ...
    def _get_all_word_shingles(
        self, wordArr, max_word_len=None, create_suffix_end=True
    ):
        ...
        if lenWordArr > compound_word_step:
            ...
            wordArr1 = [
                w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr1
            ]
            wordArr2 = [
                w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr2
            ]
            ...
            if create_suffix_end:
                ...
                wordArr3[-1] = (
                    wordArr3[-1]
                    if len(wordArr3[-1]) <= max_word_len
                    else "*" + wordArr3[-1][len(wordArr3[-1]) - max_word_len + 1 :]
                )
                wordArr4 = copy.copy(wordArr2)
                wordArr4[-1] = (
                    wordArr4[-1]
                    if len(wordArr4[-1]) <= max_word_len
                    else "*" + wordArr4[-1][len(wordArr4[-1]) - max_word_len + 1 :]
                )
                wordArr3 = [
                    w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr3
                ]
                wordArr4 = [
                    w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr4
                ]
                ret.extend([tuple(wordArr3), tuple(wordArr4)])
        else:  # lenWordArr <= compound_word_step
            wordArr1 = [
                w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr
            ]
            ...
            if lenWordArr > 1 and create_suffix_end:
                ...
                wordArr2[-1] = (
                    wordArr2[-1]
                    if len(wordArr2[-1]) <= max_word_len
                    else "*" + wordArr2[-1][len(wordArr2[-1]) - max_word_len + 1 :]
                )
                wordArr2 = [
                    w if len(w) <= max_word_len else w[:max_word_len] for w in wordArr2
                ]
    ...
    def add_to_ontology(self, word2ner, max_word_len=None, onto_name=None):
        ...
        if onto_name == self.x_lingual_onto_name:
            self.x_lingual_lexicon_by_prefix = ontology = self.ontology[
                onto_name
            ] = self.ontology.get(onto_name, self.x_lingual_lexicon_by_prefix)
        else:
        ...
        for _idx, word_label in enumerate(word2ner):
            ...
            for wordArr in self._get_all_word_shingles(
                wordArr, max_word_len=max_word_len, create_suffix_end=_idx % 5 == 0
            ):
                if not wordArr:
                    continue
                word = connector.join(wordArr)
    ...
    def in_ontology(
        ...
        word,
        connector=None,
        supress_cjk_tokenize=False,
        check_person_org_caps=True,
    ):
        ...
        long_shingles = self._get_all_word_shingles(
            wordArr, max_word_len=100000, create_suffix_end=False
        )
        for ontology in reversed(list(self.ontology.values())):
            # find patterned variations (shingles)
            for shingleArr in (
                long_shingles + all_shingles
            ):  # we can probably dedup to make it faster
                if shingleArr and shingleArr[0] in ontology:
                    lexicon2 = ontology[shingleArr[0]][
                        2 + min(3, lenWordArr // (compound_word_step + 1))
                    ]
                    if len(shingleArr) > 1:
                    ...
                    if label is not None and (
                        label[0] in self.upper_ontology
                        or self.label2label.get(label[0]) in self.upper_ontology
                    ):
                        if check_person_org_caps and (
                            "PERSON" in label or "ORG" in label
                        ):
                            # ideally we would keep patterns like AaA as part of the shingle to match. This is a hack.
                            if (
                                wordArr[0][0] != wordArr[0][0].upper()
            ...
            if ngram_start > 0:
                for j in range(ngram_start - 1, ngram_end - 2, -1):
                    if len_sent - i > j:
                        ...
                        if not self._has_nonstopword(wordArr):
                            break
                        # we don't match sequences that start and end with stopwords
                        if wordArr[-1].lower() in self.stopwords:
                            continue
                        _, label = self.in_ontology(
                            new_word, connector=connector, supress_cjk_tokenize=True
                        )
