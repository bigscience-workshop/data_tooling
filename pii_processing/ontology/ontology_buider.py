...
from data_tooling.pii_processing.ontology.ontology_builder_data import (
    OntologyBuilderData,
)
...
class OntologyBuilder(OntologyManager, OntologyBuilderData):
    ...
    def load_cn_data(self):
        ...
        if not os.path.exists(f"{tmp_dir}/conceptnet-assertions-5.7.0.csv"):
            if not os.path.exists(f"{tmp_dir}/conceptnet-assertions-5.7.0.csv.gz"):
                os.system(
                    f"wget https://s3.amazonaws.com/conceptnet/downloads/2019/edges/conceptnet-assertions-5.7.0.csv.gz"
                )
                os.system(f"mv ./conceptnet-assertions-5.7.0.csv.gz {tmp_dir}")
            os.system(f"gzip -d {tmp_dir}/conceptnet-assertions-5.7.0.csv.gz")

    def create_wn_cat(self, keep_percentage=0.01):
        """
        ...
        if not os.path.exists(f"{tmp_dir}/wn.csv"):
            os.system(
                fr"grep '\/wn\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/wn.csv"
            )
        wn = open(f"{tmp_dir}/wn.csv", "rb").read().decode().split("\n")
        wn = [s.split("\t")[0] for s in wn]
        wn = [s.strip("]").split(",/c/")[1:] for s in wn]

        wn_list = itertools.chain(
            *[
                [
                    w.split("/")[1] if w.split("/")[-2] == "wn" else w.split("/")[-2]
                    for w in awn
                    if "_" in w
                ]
                for awn in wn
        ...
        wn_list2 = itertools.chain(
            *[
                [
                    ...
                    and (w.split("/")[0] != "en")
                    and ("_" in w.split("/")[1])
                    and (w.split("/")[-2] in typeHash)
                ]
                for awn in wn
            ]
        )
        items2 = list(
            Counter([w for w in wn_list2 if w[0] not in "0123456789"]).items()
        )
    ...
    def create_rel(self):
        ...
        if not os.path.exists(f"{tmp_dir}/syn.csv"):
            os.system(
                fr"grep '\/r\/Synonym\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/syn.csv"
            )
        if not os.path.exists(f"{tmp_dir}/sim.csv"):
            os.system(
                fr"grep 'SimilarTo\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/sim.csv"
            )
        if not os.path.exists(f"{tmp_dir}/deri.csv"):
            os.system(
                fr"grep 'DerivedFrom\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/deri.csv"
            )
        if not os.path.exists(f"{tmp_dir}/erel.csv"):
            os.system(
                fr"grep 'EtymologicallyRelatedTo\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/erel.csv"
            )
        if not os.path.exists(f"{tmp_dir}/ederi.csv"):
            os.system(
                fr"grep 'EtymologicallyDerivedFrom\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/ederi.csv"
            )
        if not os.path.exists(f"{tmp_dir}/rel.csv"):
            os.system(
                fr"grep 'RelatedTo\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/rel.csv"
            )
        if not os.path.exists(f"{tmp_dir}/formof.csv"):
            os.system(
                fr"grep 'FormOf\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/formof.csv"
            )
        if not os.path.exists(f"{tmp_dir}/isa.csv"):
            os.system(
                fr"grep 'IsA\/' {tmp_dir}/conceptnet-assertions-5.7.0.csv > {tmp_dir}/isa.csv"
            )
        rel2 = OrderedDict()
        for rel_type in (
            "syn",
    ...
    def create_cn_ontology(self):
        ...
        if os.path.exists(f"{tmp_dir}/conceptnet_ontology.json"):
            return json.load(
                open(f"{tmp_dir}/conceptnet_ontology.json", "rb")
            ), json.load(open(f"{tmp_dir}/conceptnet_ontology_cat2word.json", "rb"))
                ...
                val2 = [
                    (
                        ...
                        if v in word2wncat
                            ...
                            word2wncat[v.split("_")[0]]
                            if v.split("_")[0] in word2wncat
                            else word2wncat[v.split("_")[-1]]
                        ),
                        v,
                    )
                    for v in val
                    if (v in word2wncat)
                    or (v.split("_")[0] in word2wncat)
                    or (v.split("_")[-1] in word2wncat)
                ]
                if val2:
                ...
                for idx2, word in enumerate(copy_val2):
                    ...
                    for idx in range(idx2 + 1, len_val2):
                        if type(val2[idx]) is tuple:
                            continue
                        if type(val2[idx]) is str and (
                            word in val2[idx]
                            or val2[idx].startswith(word[:-1])
                            or val2[idx].startswith(word[:-2])
                            or val2[idx].startswith(word[:-3])
                            or val2[idx].endswith(word[1:])
                            or val2[idx].endswith(word[2:])
                            or val2[idx].endswith(word[2:])
                        ):
        ...
        json.dump(
            rel2,
            open(f"{tmp_dir}/conceptnet_ontology.json", "w", encoding="utf8"),
            indent=1,
        )
        json.dump(
            cat2word,
            open(f"{tmp_dir}/conceptnet_ontology_cat2word.json", "w", encoding="utf8"),
            indent=1,
        )
    ...
    def create_eng2multilang_dict(self):
        tmp_dir = self.tmp_dir
        if hasattr(self, "word2lang") and self.word2lang:
            return
        if os.path.exists(f"{tmp_dir}/conceptnet_en.json"):
            ...
            self.word2lang = json.load(
                open(f"{tmp_dir}/conceptnet_word2lang.json", "rb")
            )
            return
        self.load_cn_data()
        if not os.path.exists(f"{tmp_dir}/syn.csv"):
        ...
        for rel_type in ("syn",):
            ...
            for s in rel:
                ...
                b = b[1]
                word2lang[
                    a.replace(" ", "_").replace("-", "_").lower().strip(".")
                ] = list(
                    set(
                        word2lang.get(
                            a.replace(" ", "_").replace("-", "_").lower().strip("."), []
                ...
                word2lang[
                    b.replace(" ", "_").replace("-", "_").lower().strip(".")
                ] = list(
                    set(
                        word2lang.get(
                            b.replace(" ", "_").replace("-", "_").lower().strip("."), []
                ...
                if lang1 in ("zh", "ja", "ko") and self.cjk_detect(a):
                    a = (
                        "_".join(mt5_tok.tokenize(a))
                        .replace(mt5_underscore, "_")
                        .replace("__", "_")
                        .replace("__", "_")
                        .strip("_")
                    )
        ...
        for key in list(self.word2en.keys()):
            self.word2en[key] = {a: 1 for a in self.word2en[key]}
        json.dump(
            self.en,
            open(f"{tmp_dir}/conceptnet_en.json", "w", encoding="utf8"),
            indent=1,
        )
        json.dump(
            self.word2en,
            open(f"{tmp_dir}/conceptnet_word2en.json", "w", encoding="utf8"),
            indent=1,
        )
        json.dump(
            self.word2lang,
            open(f"{tmp_dir}/conceptnet_word2lang.json", "w", encoding="utf8"),
            indent=1,
        )
    ...
    def yago_step0(self):
        ...
        if not os.path.exists(f"{tmp_dir}/yago-wd-simple-types.nt.gz"):
            os.system(
                "wget https://yago-knowledge.org/data/yago4/full/2020-02-24/yago-wd-simple-types.nt.gz"
            )
        ...
        with gzip.open(f"{tmp_dir}/yago-wd-simple-types.nt.gz") as f:  # gzip.
            with open(f"{tmp_dir}/yago0.tsv", "w", encoding="utf8") as o:
                while True:
                    ...
                    line = line.decode()
                    entity, rel, cat, _ = [
                        l.split("/")[-1].strip(" .>") for l in line.split(">")
                    ]
                    if "_" in entity:
                        entityArr = entity.split("_")
                        _id = entityArr[-1]
                        entity = aHash[_id] = urllib.parse.unquote(
                            "_".join(entityArr[:-1])
                            .split("(")[0]
                            .lower()
                            .replace("-", "_")
            ...
            with open(f"{tmp_dir}/yago_problem.tsv", "w", encoding="utf8") as p:
                for _idx in range(2):
                    ...
                    with open(f"{tmp_dir}/yago{_idx}.tsv", "rb") as f:
                        _idx += 1
                        with open(
                            f"{tmp_dir}/yago{_idx}.tsv", "w", encoding="utf8"
                        ) as o:
                            while True:
                                line = f.readline()
                                if not line:
                                ...
                                if prev_entity and entity != prev_entity:
                                    if (
                                        ...
                                        cats = [
                                            a
                                            for a in Counter(cats).most_common()
                                            if a[0] != "THING"
                                        ]
                                        if cats and prev_entity.count("_") <= 4:
                                            o.write(
                                                prev_entity.translate(trannum)
                                                + "\t"
                                                + cats[0][0]
                                                + "\n"
                                            )
                            ...
                            if cats:
                                if (
                                    ...
                                    and not (
                                        prev_entity.count("_") <= 4
                                        and (
                                            ":" in prev_entity or prev_entity[-1] == "1"
                                        )
                                    )
                                ):
                                    a.write(prev_entity.translate(trannum) + "\n")
                    ...
                    os.system(
                        f"sort --parallel=32 {tmp_dir}/yago{_idx}.tsv --o {tmp_dir}/yago{_idx}.tsv"
                    )
    ...
    def yago_step2(self):
        ...
        yago_dict = dict(
            [
                a.split("\t")
                for a in open(f"{tmp_dir}/yago_ontology.tsv", "rb")
                .read()
                .decode()
                .split("\n")
                if len(a.split("\t")) == 2
        ...
        for word, label in yago_dict.items():
            ...
            if synset is not None:
                ...
                if label in ("MEDICAL_THERAPY",):
                    ...
                    if label == "JOB" and (
                        scientist in hype or leader in hype or capitalist in hype
                    ):
                        new_yago_ontology[word] = label
                        continue
                    if person in hype:
                ...
                if (
                    ...
                    or symptom in hype
                    or location in hype
                    or condition in hype
                    or body_part in hype
                    or substance in hype
                    or food in hype
                    or act in hype
                    or process in hype
                ):
        ...
        json.dump(
            lst, open(f"{tmp_dir}/yago_ontology.json", "w", encoding="utf8"), indent=1
        )
    ...
    def create_combined_cn_yago(self):
        ...
        tmp_dir = self.tmp_dir
        if hasattr(self, "ner2word") and self.ner2word:
            return
        if os.path.exists(f"{tmp_dir}/conceptnet_yago_combined.json"):
            self.ner2word = json.load(
                open(f"{tmp_dir}/conceptnet_yago_combined.json", "rb")
            )
            self.yago2ner = [
                tuple(a) for a in json.load(open(f"{tmp_dir}/yago2ner.json", "rb"))
            ]
            return
        ...
        cat2word_list = list(
            json.load(open(f"{tmp_dir}/conceptnet_ontology_cat2word.json")).items()
        )
        word2ner = {}
        ner2word = {}
        ...
        for cat, words in cat2word_list:
            ...
            if label:
                for word in words:
                    if self.cjk_detect(word):
                        word = (
                            "_".join(mt5_tok.tokenize(word.replace("_", "")))
                            .replace(mt5_underscore, "_")
                            .replace("__", "_")
                            .replace("__", "_")
                            .strip("_")
        ...
        for word, label in yago0:
            if word in word2ner or word in self.word2en:
                if (
                    label == "PERSON"
                    and word in word2ner
                    and word2ner[word] == "PUBLIC_FIGURE"
                ):
                    continue
                word2ner[word] = label
            else:
        ...
        json.dump(
            ner2word,
            open(f"{tmp_dir}/conceptnet_yago_combined.json", "w", encoding="utf8"),
            indent=1,
        )
        json.dump(
            yago2ner, open(f"{tmp_dir}/yago2ner.json", "w", encoding="utf8"), indent=1
        )
        self.ner2word = ner2word
        self.yago2ner = yago2ner

    def save_cross_lingual_ontology(
        self, word2ner_file="word2ner.json", do_small_set=True
    ):
        tmp_dir = self.tmp_dir
        data_dir = self.data_dir
        self.create_eng2multilang_dict()
        ...
        product_en = list({a for a in ner2word["PRODUCT"] if a.count("_") > 0})
        medical_therapy_en = list(
            {a for a in ner2word["MEDICAL_THERAPY"] if a.count("_") > 0}
        )
        plant_en = list({a for a in ner2word["PLANT"] if a.count("_") > 0})
        animal_en = list({a for a in ner2word["ANIMAL"] if a.count("_") > 0})
        work_of_art_en = (
            []
        )  # list(set(a for a in ner2word['WORK_OF_ART'] if a.split("_")[0].lower() not in self.stopwords and a.count("_") > 0 and  a.count("_") < 5))
        gpe_en = list({a for a in ner2word["GPE"] if a.count("_") > 0})
        event_en = list({a for a in ner2word["EVENT"] if a.count("_") > 0})
        language_en = list(set(ner2word["LANGUAGE"]))
        disease_en = list({a for a in ner2word["DISEASE"] if a.count("_") > 0})
        org_en = list({a for a in ner2word["ORG"] if a.count("_") > 0})
        date_en = list({a for a in ner2word["DATE"] if a.count("_") > 0})
        law_en = list({a for a in ner2word["LAW"] if a.count("_") > 0})
        food_en = list({a for a in ner2word["FOOD"] if a.count("_") > 0})
        domain_name_en = list(
            {a for a in ner2word.get("DOMAIN_NAME", []) if a.count("_")}
        )
        quantity_en = list({a for a in ner2word["QUANTITY"] if a.count("_") > 0})
        union_en = list({a for a in ner2word.get("UNION", []) if a.count("_") > 0})
        fac_en = list({a for a in ner2word["FAC"] if a.count("_") > 0})
        location_en = list({a for a in ner2word["LOCATION"] if a.count("_") > 0})
        bio_chem_en = list({a for a in ner2word["BIO_CHEM_ENTITY"] if a.count("_") > 0})
        ...
        block_list = {
            ...
            "knowledge",
            "power",
            "structuralhypothesistypebystructuretype",
            "emission",
            "harper",
            "fighter",
            "g",
        ...
        lst = [
            (
                word,
                (1.0 + sum(a.count("_") for a in self.word2en.get(word, [])))
                / (1.0 + len(self.word2en.get(word, [])))
                - 1.0,
            )
            for word in ner2word["PUBLIC_FIGURE"]
        ]
        lst = [
            l
            for l in lst
            ...
            and not [
                a
                for a in self.word2en.get(l[0], {l[0]})
                if a.split("_")[-1] in block_list
            ]
            and not [
                a
                for a in self.word2en.get(l[0], {l[0]})
                if a.split("_")[0] in set(list(block_list) + list(block_list2))
            ]
        ]
        lst = lst + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("person.n.01").closure(lambda s: s.hyponyms())
                    if not d.hyponyms()
                ]
            )
            if a[0] == a[0].upper()
        ]
        public_figure_en = list(
            set(
                OntologyBuilder.public_figure_list
                + list(itertools.chain(*[self.word2en.get(l[0], {l[0]}) for l in lst]))
            )
        )
        lst = [
            (
        ...
        person_en = list(
            itertools.chain(*[self.word2en.get(l[0], {l[0]}) for l in lst])
        )

        union_en = (
            union_en
            + [
                word.replace(" ", "_").replace("-", "_").lower().strip(".")
                for word in OntologyBuilder.union_list
            ]
            + [
                a
                for a in itertools.chain(
                    *[
                        [str(a.name()) for a in d.lemmas()]
                        for d in wn.synset("union.n.01").closure(lambda s: s.hyponyms())
                        if not d.hyponyms()
                    ]
                )
            ]
        )
        social_economic_class_en = OntologyBuilder.soc_eco_class_list + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("class.n.03").closure(lambda s: s.hyponyms())
                    if not d.hyponyms()
                ]
            )
        ]
        professional_en = [
            ...
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("professional.n.01").closure(
                        lambda s: s.hyponyms()
                    )
                    if not d.hyponyms()
                ]
        ...
        worker_en = [
            ...
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("worker.n.01").closure(lambda s: s.hyponyms())
                    if not d.hyponyms()
                    if not d.hyponyms()
                ]
            )
        ...
        poltical_party_member_en = self.political_party_member_list + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("politician.n.02").closure(
                        lambda s: s.hyponyms()
                    )
                    if not d.hyponyms()
                ]
            )
            if a[0] == a[0].upper() and "_" not in a
        ]
        politcal_party_en = OntologyBuilder.political_party_list + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("party.n.01").closure(lambda s: s.hyponyms())
                    if not d.hyponyms()
                ]
            )
            if a[0] == a[0].upper()
        ...
        organization_en = (
            org_en
            + OntologyBuilder.org_list
            + [
                a
                for a in itertools.chain(
                    *[
                        [str(a.name()) for a in d.lemmas()]
                        for d in wn.synset("organization.n.01").closure(
                            lambda s: s.hyponyms()
                        )
                        if not d.hyponyms()
                    ]
        ...
        religion_list_en = [
            word.replace(" ", "_").replace("-", "_").lower().strip(".")
            for word in OntologyBuilder.person2religion.values()
        ] + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("religion.n.01").closure(lambda s: s.hyponyms())
                    if not d.hyponyms()
                ]
            )
            if a[0] == a[0].upper()
        ...
        gender_person_en = OntologyBuilder.gender_list + [
            word.replace(" ", "_").replace("-", "_").lower().strip(".")
            for word in itertools.chain(*OntologyBuilder.pronoun2gender.values())
        ]

        language_list_en = list(
            set(
                language_en
                + [
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in person.Provider.language_names
                ]
            )
        )
        ...
        race_list_en = list(
            {
                word.replace(" ", "_").replace("-", "_").lower().strip(".")
                for word in OntologyBuilder.race_list
            }
        )
        race_list2 = []
        ...
        for word in race_list_en + language_list_en:
            for gender in [
                ...
                for word2 in [
                    gender + "_" + word,
                    word + "_" + gender,
                    "older_" + gender + "_" + word,
                    "old_" + word + "_" + gender,
                    "younger_" + gender + "_" + word,
                    "young_" + word + "_" + gender,
                    "adult_" + word + "_" + gender,
                    "senior_" + word + "_" + gender,
                ]:
                    if word2 in self.en:
                        race_list2.append(word2)

        religious_person_en = list(
            set(
                OntologyBuilder.religious_member_list
                + [
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in [
                        word.split(",")[0].strip()
                        for word in OntologyBuilder.person2religion.keys()
                    ]
                ]
            )
        )
        religious_person_en = religious_person_en + [
            a
            for a in itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("religious_person.n.01").closure(
                        lambda s: s.hyponyms()
                    )
                ]
            )
            if a[0] == a[0].upper() and a not in ("WASP",)
        ]
        religious_person2 = []
        ...
        jobs_en = (
            professional_en
            + worker_en
            + OntologyBuilder.jobs
            + list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in [
                        word.split(",")[0].strip() for word in job.Provider.jobs
                    ]
                }
            )
        )
        ...
        disease_list_en = (
            ...
            + list(
                itertools.chain(
                    *[
                        [str(a.name()) for a in d.lemmas()]
                        for d in wn.synset("physical_condition.n.01").closure(
                            lambda s: s.hyponyms()
                        )
                    ]
                )
        ...
        disease_list_en = disease_list_en + list(
            itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("infectious_agent.n.01").closure(
                        lambda s: s.hyponyms()
                    )
                ]
            )
        ...
        disease_list_en = disease_list_en + list(
            itertools.chain(
                *[
                    [str(a.name()) for a in d.lemmas()]
                    for d in wn.synset("symptom.n.01").closure(lambda s: s.hyponyms())
                ]
            )
        )
        ...
        if "GENDER" not in ignore_ner_labels:
            (
                ...
                word2ner,
                block_list,
            ) = self.create_multilingual_examples(
                gender_person_en,
                "GENDER",
                word2ner,
        ...
        print("num of items", len(word2ner))
        title_list, word2ner, block_list = self.create_multilingual_examples(
            self.title_list,
            "TITLE",
            word2ner,
            cut_off_per=0.2,
        ...
        person_pronoun_list, word2ner, block_list = self.create_multilingual_examples(
            self.person_pronoun_list,
            "PERSON_PRONOUN",
            word2ner,
            cut_off_per=0.2,
            block_list=block_list
            + [
                "interjection_expressing_anger_or_chagrin",
                "i_translations",
                "information_technology",
                "here",
            ]
            + self.other_pronoun_list,
        )
        other_pronoun_list, word2ner, block_list = self.create_multilingual_examples(
            self.other_pronoun_list,
            "OTHER_PRONOUN",
            word2ner,
            cut_off_per=0.2,
        ...
        domain_name_list, word2ner, block_list = self.create_multilingual_examples(
            domain_name_en, "DOMAIN_NAME", word2ner, block_list=block_list
        )
        quantity_list, word2ner, block_list = self.create_multilingual_examples(
            quantity_en,
            "QUANTITY",
            word2ner,
            block_list=block_list
        ...
        bio_chem_list, word2ner, block_list = self.create_multilingual_examples(
            bio_chem_en, "BIO_CHEM_ENTITY", word2ner, block_list=block_list
        )
        anat_list, word2ner, block_list = self.create_multilingual_examples(
            anat_en, "ANAT", word2ner, block_list=block_list
        )
        medical_therapy_list, word2ner, block_list = self.create_multilingual_examples(
            medical_therapy_en, "MEDICAL_THERAPY", word2ner, block_list=block_list
        )
        plant_list, word2ner, block_list = self.create_multilingual_examples(
            plant_en,
            "PLANT",
            word2ner,
            block_list=block_list
        ...
        print("num of items", len(word2ner))
        animal_list, word2ner, block_list = self.create_multilingual_examples(
            animal_en,
            "ANIMAL",
            word2ner,
            block_list=block_list
            + [
                ...
                "blockhead",
                "nonsense",
                "something_strange_or_suspicious",
                "ester",
                "who",
                "ase",
                "moke",
        ...
        gpe_list, word2ner, block_list = self.create_multilingual_examples(
            gpe_en,
            "GPE",
            word2ner,
            block_list=block_list
        ...
        fac_list, word2ner, block_list = self.create_multilingual_examples(
            fac_en,
            "FAC",
            word2ner,
            block_list=block_list
        ...
        location_list, word2ner, block_list = self.create_multilingual_examples(
            location_en,
            "LOCATION",
            word2ner,
            block_list=block_list
        ...
        event_list, word2ner, block_list = self.create_multilingual_examples(
            event_en, "EVENT", word2ner, block_list=block_list
        )
        date_list, word2ner, block_list = self.create_multilingual_examples(
            date_en,
            "DATE",
            word2ner,
            block_list=block_list
        ...
        law_list, word2ner, block_list = self.create_multilingual_examples(
            law_en,
            "LAW",
            word2ner,
            block_list=block_list
        ...
        food_list, word2ner, block_list = self.create_multilingual_examples(
            food_en,
            "FOOD",
            word2ner,
            block_list=block_list
        ...
        print("num of items", len(word2ner))
        language_list, word2ner, block_list = self.create_multilingual_examples(
            language_list_en,
            "LANGUAGE",
            word2ner,
            block_list=block_list
        ...
        job_list, word2ner, block_list = self.create_multilingual_examples(
            jobs_en,
            "JOB",
            word2ner,
            block_list=block_list
            + [
                ...
                "artillerist",
                "witch_doctor",
                "person_who_gives_treatment",
                "person_who_reads_cards",
                "copper",
                "bull",
                "führer",
                "sophisticate",
        ...
        race_list, word2ner, block_list = self.create_multilingual_examples(
            race_list_en,
            "RACE",
            word2ner,
            block_list=block_list
        ...
        religious_member_list, word2ner, block_list = self.create_multilingual_examples(
            religious_person_en,
            "RELIGION_MEMBER",
            word2ner,
            block_list=block_list
        ...
        religion_list, word2ner, block_list = self.create_multilingual_examples(
            religion_list_en,
            "RELIGION",
            word2ner,
            block_list=block_list
        ...
        union_list, word2ner, block_list = self.create_multilingual_examples(
            union_en,
            "UNION",
            word2ner,
            block_list=block_list
        ...
        (
            social_economic_class_list,
            word2ner,
            block_list,
        ) = self.create_multilingual_examples(
            social_economic_class_en,
            "SOC_ECO_CLASS",
            word2ner,
            block_list=block_list
            + [
        ...
        political_party_list, word2ner, block_list = self.create_multilingual_examples(
            politcal_party_en,
            "POLITICAL_PARTY",
            word2ner,
            block_list=block_list
            + ["ungradable_opposition", "work", "time", "service"],
        )
        (
            political_party_member_list,
            word2ner,
            block_list,
        ) = self.create_multilingual_examples(
            poltical_party_member_en,
            "POLITICAL_PARTY_MEMBER",
            word2ner,
            block_list=["red", "rouge", "communistically"],
        )
        public_figure_list, word2ner, block_list = self.create_multilingual_examples(
            public_figure_en,
            "PUBLIC_FIGURE",
            word2ner,
            block_list=block_list
        ...
        person_list, word2ner, block_list = self.create_multilingual_examples(
            person_en,
            "PERSON",
            word2ner,
            block_list=block_list
        ...
        org_list, word2ner, block_list = self.create_multilingual_examples(
            organization_en,
            "ORG",
            word2ner,
            block_list=block_list
            + [
                ...
                "us",
                "americas",
                "normalized_projection_coordinates",
                ...
                "weak",
                "doctor",
                "first_lord_of_treasury",
                "sixth_form",
                "peddlers_and_carriers",
                "utility",
                "gallery",
                "fiction",
                "st",
        ...
        print("num of items", len(word2ner))
        disease_list, word2ner, block_list = self.create_multilingual_examples(
            disease_list_en,
            "DISEASE",
            word2ner,
            block_list=block_list
            + [
                ...
                "shrew",
                "hiccups",
                "abnormally_high_blood_sugar_level",
                "weak_digestion",
                "mule",
                "bitterness",
                "ketose",
                ...
                "shave",
                "ferociousness",
                "reproductive_procreative_power",
                "fruitfulness",
                "fertility_rate",
                "bloating",
                "meteorism",
        ...
        product_list, word2ner, block_list = self.create_multilingual_examples(
            product_en,
            "PRODUCT",
            word2ner,
            block_list=block_list
        ...
        self.word2ner = word2ner = list(
            {(word.translate(trannum), label) for word, label in word2ner}
        )
        print("num of items", len(word2ner))
        os.system(f"mkdir -p {tmp_dir}")
        json.dump(
            word2ner, open(f"{tmp_dir}/{word2ner_file}", "w", encoding="utf8"), indent=1
        )
        ...
        male_to_female_gender_swap = copy.copy(
            OntologyBuilder.male_to_female_gender_swap
        )
        ...
        for lang in OntologyBuilder.faker_list:
                ...
                if not type(provider.Provider.formats) is dict:
                    ner_regexes += [
                        (
                            ...
                            "|".join(
                                [
                                    a.replace("{{", "<").replace("}}", r">\d+").upper()
                                    for a in provider.Provider.formats
                                ]
                            ),
                            False,
                            (),
                    ...
                    ner_regexes += [
                        (
                            ...
                            "|".join(
                                [
                                    a.replace("{{", "<").replace("}}", r">\d+").upper()
                                    for a in provider.Provider.formats.keys()
                                ]
                            ),
                            False,
                            (),
            ...
            if hasattr(provider.Provider, "first_names_female"):
                if not type(provider.Provider.first_names_female) is dict:
                    aHash["FIRST_NAME_FEMALE"] = list(
                        set(
                            aHash.get("FIRST_NAME_FEMALE", [])
                            + list(provider.Provider.first_names_female)
                        )
                    )
                else:
                    aHash["FIRST_NAME_FEMALE"] = list(
                        set(
                            aHash.get("FIRST_NAME_FEMALE", [])
                            + list(provider.Provider.first_names_female.keys())
                        )
                    )
            if hasattr(provider.Provider, "first_names_male"):
                if not type(provider.Provider.first_names_female) is dict:
                    aHash["FIRST_NAME_MALE"] = list(
                        set(
                            aHash.get("FIRST_NAME_MALE", [])
                            + list(provider.Provider.first_names_male)
                        )
                    )
                else:
                    aHash["FIRST_NAME_MALE"] = list(
                        set(
                            aHash.get("FIRST_NAME_MALE", [])
                            + list(provider.Provider.first_names_male.keys())
                        )
                    )
            if hasattr(provider.Provider, "last_names_female"):
                if not type(provider.Provider.last_names_female) is dict:
                    aHash["LAST_NAMES_FEMALE"] = list(
                        set(
                            aHash.get("LAST_NAMES_FEMALE", [])
                            + list(provider.Provider.last_names_female)
                        )
                    )
                else:
                    aHash["LAST_NAMES_FEMALE"] = list(
                        set(
                            aHash.get("LAST_NAMES_FEMALE", [])
                            + list(provider.Provider.last_names_female.keys())
                        )
                    )
            if hasattr(provider.Provider, "LAST_NAMES_MALE"):
                if not type(provider.Provider.last_names_male) is dict:
                    aHash["LAST_NAMES_MALE"] = list(
                        set(
                            aHash.get("LAST_NAMES_MALE", [])
                            + list(provider.Provider.last_names_male)
                        )
                    )
                else:
                    aHash["LAST_NAMES_MALE"] = list(
                        set(
                            aHash.get("LAST_NAMES_MALE", [])
                            + list(provider.Provider.last_names_male.keys())
                        )
                    )
            if hasattr(provider.Provider, "prefixes_female"):
                if not type(provider.Provider.prefixes_male) is dict:
                    aHash["PREFIX_FEMALE"] = list(
                        set(
                            aHash.get("PREFIX_FEMALE", [])
                            + list(provider.Provider.prefixes_female)
                        )
                    )
                else:
                    aHash["PREFIX_FEMALE"] = list(
                        set(
                            aHash.get("PREFIX_FEMALE", [])
                            + list(provider.Provider.prefixes_female.keys())
                        )
                    )
            if hasattr(provider.Provider, "prefixes_male"):
                if not type(provider.Provider.prefixes_male) is dict:
                    aHash["PREFIX_MALE"] = list(
                        set(
                            aHash.get("PREFIX_MALE", [])
                            + list(provider.Provider.prefixes_male)
                        )
                    )
                else:
                    aHash["PREFIX_MALE"] = list(
                        set(
                            aHash.get("PREFIX_MALE", [])
                            + list(provider.Provider.prefixes_male.keys())
                        )
                    )

            if hasattr(provider.Provider, "sufixes_female"):
                if not type(provider.Provider.sufixes_male) is dict:
                    aHash["SUFIX_FEMALE"] = list(
                        set(
                            aHash.get("SUFIX_FEMALE", [])
                            + list(provider.Provider.sufixes_female)
                        )
                    )
                else:
                    aHash["SUFIX_FEMALE"] = list(
                        set(
                            aHash.get("SUFIX_FEMALE", [])
                            + list(provider.Provider.sufixes_female.keys())
                        )
                    )
            if hasattr(provider.Provider, "sufixes_male"):
                if not type(provider.Provider.sufixes_male) is dict:
                    aHash["SUFIX_MALE"] = list(
                        set(
                            aHash.get("SUFIX_MALE", [])
                            + list(provider.Provider.sufixes_male)
                        )
                    )
                else:
                    aHash["SUFIX_MALE"] = list(
                        set(
                            aHash.get("SUFIX_MALE", [])
                            + list(provider.Provider.sufixes_male.keys())
                        )
                    )

            if hasattr(provider.Provider, "first_names"):
                if not type(provider.Provider.first_names) is dict:
                    aHash["FIRST_NAME"] = list(
                        set(
                            aHash.get("FIRST_NAME", [])
                            + list(provider.Provider.first_names)
                        )
                    )
                else:
                    aHash["FIRST_NAME"] = list(
                        set(
                            aHash.get("FIRST_NAME", [])
                            + list(provider.Provider.first_names.keys())
                        )
                    )
            if hasattr(provider.Provider, "last_names"):
                if not type(provider.Provider.last_names) is dict:
                    aHash["LAST_NAME"] = list(
                        set(
                            aHash.get("LAST_NAME", [])
                            + list(provider.Provider.last_names)
                        )
                    )
                else:
                    aHash["LAST_NAME"] = list(
                        set(
                            aHash.get("LAST_NAME", [])
                            + list(provider.Provider.last_names.keys())
                        )
                    )
        ...
        langs = list(
            set(
                list(lang2person.keys())
                + list(binary_gender_swap.keys())
                + list(other_gender_swap.keys())
                + list(en_pronoun2gender.keys())
                + list(en_pronoun2pronoun.keys())
                + list(person2religion.keys())
            )
        )
        for lang in langs:
            if lang in ("sw",):
                ret["LAST_NAME"] = list(
                    set(
                        ...
                        + [
                            word.replace(" ", "_").replace("-", "_").lower().strip(".")
                            for word in OntologyBuilder.bantu_surnames
                        ]
                    )
                )
            ...
            for key, val in personHash.items():
                ...
                if key in ret:
                    ...
                    ret[key] = val
            ret["FIRST_NAME_MALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("FIRST_NAME_MALE", [])
                }
            )
            ret["LAST_NAME_MALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("LAST_NAME_MALE", [])
                }
            )
            ret["FIRST_NAME_FEMALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("FIRST_NAME_FEMALE", [])
                }
            )
            ret["LAST_NAME_FEMALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("LAST_NAME_FEMALE", [])
                }
            )
            ret["FIRST_NAME"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("FIRST_NAME", [])
                }
            )
            ret["LAST_NAME"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("LAST_NAME", [])
                }
            )
            ret["SUFIX_MALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("SUFIX_MALE", [])
                }
            )
            ret["SUFIX_FEMALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in ret.get("SUFIX_FEMALE", [])
                }
            )
            ...
            ret["PREFIX_MALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in en_pronoun2title.get("he", [])
                    + ret.get("PREFIX_MALE", [])
                }
            )
            ret["PREFIX_FEMALE"] = list(
                {
                    word.replace(" ", "_").replace("-", "_").lower().strip(".")
                    for word in en_pronoun2title.get("she", [])
                    + ret.get("PREFIX_FEMALE", [])
                }
            )
            ret["person2religion"] = person2religion.get(lang, {})
            if lang == "en":
                ret["ner_regexes"] = (
                    ret.get("ner_regexes", []) + OntologyBuilder.default_ner_regexes
                )
            json.dump(
                ret, open(f"{data_dir}/{lang}.json", "w", encoding="utf8"), indent=1
            )
    ...
    def word2cat(self):
        tmp_dir = self.tmp_dir
        cat2word = json.load(
            open(f"{tmp_dir}/conceptnet_ontology_cat2word.json", "rb").decode()
        )
        from collections import Counter
    ...
    def create_multilingual_map(
        ...
        allow_list = {a: 1 for a in allow_list or []}
        ret_hash = {}
        print(keys)
        if not keys:
            return ret_hash
        if type(en_examples[keys[0]]) is dict:
            for item in keys:
                ...
                for key in mapHash.keys():
                    ...
                    for word in words:
                        ...
                        words2 = [
                            word2
                            for word2 in en[word]
                            if len(word2en[word2]) <= cut_off_abs
                        ]
                        if not words2:
                            continue
                        ...
                        for lang in word2lang.get(word2, []):
                            ...
                            if allow_list is not None and word2 not in allow_list:
                                continue
            ...
            for item, words in en_examples.items():
                ...
                for key2 in words:
                    ...
                    lang2words_list = itertools.chain(
                        *[
                            [
                                (lang, word2)
                                for lang in word2lang.get(
                                    word2, ["en"] if word2 == key2 else []
                                )
                            ]
                            for word2 in en.get(key2, []) + [key2]
                            if word2 == key2
                            or (word2 in word2en and len(word2en[word2])) <= cut_off_abs
                        ]
                    )
                ...
                for lang, words2 in lang2words.items():
                    aHash = ret_hash.get(lang, {})
                    for word in words2:
                        if allow_list and word not in allow_list:
                            continue
            ...
            for key in en_examples:
                ...
                lang2words_list = itertools.chain(
                    *[
                        [
                            (lang, word)
                            for lang in word2lang.get(
                                word, ["en"] if word == key else []
                            )
                        ]
                        for word in en.get(key, []) + [key]
                        if word == key
                        or (word in word2en and len(word2en[word])) <= cut_off_abs
                    ]
                )
                lang2words = {}
                ...
                lang2words["en"] = [key]
                key2 = key2.replace(" ", "_").replace("-", "_").lower().strip(".")
                lang2words2_list = itertools.chain(
                    *[
                        [
                            (lang, word2)
                            for lang in word2lang.get(
                ...
                for lang in lang2words.keys():
                    for word in lang2words[lang]:
                        ...
                        if word2 == word:
                            continue
                        if allow_list and word2 not in allow_list:
                            continue
                        aHash = ret_hash.get(lang, {})
                        aHash[word] = word2
                        ret_hash[lang] = aHash
    ...
    def create_multilingual_examples(
        self,
        en_examples,
        ner_label,
        word2ner,
        ...
        ret_list = []
        ret_hash = {
            word.replace(" ", "_").replace("-", "_").lower().strip("."): 1
            for word in en_examples
        }
        ...
        for i in range(3):
            ...
            for word in added:
                ...
                found = [
                    (
                        w,
                        [w2 for w2 in word2en[w] if w2 not in ret_hash],
                        len([w2 for w2 in word2en[w] if w2 not in ret_hash])
                        / len(word2en[w]),
                    )
                    for w in en[word]
                    if not has_any(word2en[w], block_list)
                ]
                found = [
                    a for a in found if len(a[1]) < cut_off_abs and a[2] < cut_off_per
                ]
